...
export class ModalSpecializationComponent implements OnInit {
  @Input() tc_data : {};
  @Input() _id :String;
  statusData: IOption[];
  myForm : FormGroup;
...
// Save New Record
  saveData(valid: boolean) {
    if (valid) {
      this.loadingButton = true;
        this._specializationService.create(this.myForm.value)
        .subscribe(
          response => 
          {
            if(response){
  ...
  updateData(valid: boolean) {
    if (valid) {
      this.loadingButton = true;
        this._specializationService.update(this._id,this.myForm.value)
        .subscribe(
          response => 
          {
