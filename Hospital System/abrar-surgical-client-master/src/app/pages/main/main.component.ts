...
export class MainPageComponent extends BasePageComponent implements OnInit, OnDestroy {
  ...
  indoor_bill : number;
  outdoor_bill : number;
  active_indoor : number;


  constructor(
    store: Store<IAppState>,
    private _helper_serive : HelperService,
  ...
  ngOnInit() {
    ...
    this.setHSOptions();
    //this.setPAOptions(response.age);
    //this.setPIOptions();
    //this.setHEOptions();
  }
  ...
  getAllData()
  {
    this._helper_serive.dashboard()
    .subscribe(
      response => 
      {
        this.total_patient = response.total_patient;
        this.indoor_bill  = response.indoor_bill;
        this.outdoor_bill = response.outdoor_bill;
        this.active_indoor = response.active_indoor;
        this.setHSOptions();
        this.setPAOptions(response.patient_age);
        this.setPGOptions(response.patient_male,response.patient_female);
        this.setDOptions(response.indoor_bill,response.outdoor_bill,response.active_indoor);
      },
      error => {
      console.log(error)
      });
      ...
      series: [
        {
          ...
          type: 'line',
          smooth: true,
          data: [95, 124, 132, 143, 138, 178, 194, 211, 0, 0, 0, 0]
        }
      ]
    };
  }
  ...
  setDOptions(indoor,outdoor,active_indoor) {
    this.dOptions = {
      grid: {
        left: 0,
        right: 0,
      ...
      series: [{
        ...
        data:[
          ...
          { value: active_indoor, name: 'Active Indoor' },
        ],
        itemStyle: this.pieStyle
      }]
    };
